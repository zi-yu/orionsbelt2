...
namespace OrionsBelt.Engine.Universe {
	public class BattleFleetSectorInformation : SectorInformation{
			...
			get {
                using (StringWriter writer = new StringWriter()) {
                    foreach (IFleetInfo fleet in sector.GetFleets()) {
                        writer.Write(fleet.Owner.RaceInfo.Race);
                    }
                    return string.Format("fleet{0}", writer);
                }
			}
		}
		...
		public BattleFleetSectorInformation(ISector sector)
			: base(sector) {
			FleetBattleSector fbs = (FleetBattleSector)sector;
			f1 = (IFleetInfo)fbs.CelestialBodies[0];
			f2 = (IFleetInfo)fbs.CelestialBodies[1];
			battleId = fbs.BattleId;
		}
