...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Token of the SpecialEvent entity
    /// </summary>
	public class SpecialEventTokenEditor : 
		...
		{
			if( !Secret ) {
				entity.Token = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Token = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_token", ID); }
		}
