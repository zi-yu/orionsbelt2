...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Resorces of the SpecialEvent entity
    /// </summary>
	public class SpecialEventResorcesEditor : 
					StringEditor<SpecialEvent>, INamingContainer {
		...
		protected override string GetCaption( SpecialEvent entity )
		{
			return entity.Resorces;
		}
		...
		{
			if( !Secret ) {
				entity.Resorces = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Resorces = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_resorces", ID); }
		}
