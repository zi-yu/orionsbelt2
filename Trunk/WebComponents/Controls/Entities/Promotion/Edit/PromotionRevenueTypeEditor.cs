...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the RevenueType of the Promotion entity
    /// </summary>
	public class PromotionRevenueTypeEditor : 
		...
		{
			if( !Secret ) {
				entity.RevenueType = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.RevenueType = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_revenueType", ID); }
		}
