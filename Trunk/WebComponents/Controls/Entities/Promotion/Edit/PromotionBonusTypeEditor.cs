...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the BonusType of the Promotion entity
    /// </summary>
	public class PromotionBonusTypeEditor : 
		...
		{
			if( !Secret ) {
				entity.BonusType = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.BonusType = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_bonusType", ID); }
		}
