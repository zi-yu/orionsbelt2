...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Status of the Promotion entity
    /// </summary>
	public class PromotionStatusEditor : 
		...
		{
			if( !Secret ) {
				entity.Status = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Status = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_status", ID); }
		}
