...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Description of the Necessity entity
    /// </summary>
	public class NecessityDescriptionEditor : 
					StringEditor<Necessity>, INamingContainer {
		...
		protected override string GetCaption( Necessity entity )
		{
		...
		{
			if( !Secret ) {
				entity.Description = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Description = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_description", ID); }
		}
