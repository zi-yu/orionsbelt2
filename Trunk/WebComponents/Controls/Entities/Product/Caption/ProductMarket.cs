...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// <summary>
    /// Renders in XHTML the content of a Product's Market
    /// </summary>
	public class ProductMarket : BaseFieldControl<Product>, INamingContainer {
		...
		protected override void Render( HtmlTextWriter writer, Product entity, int renderCount, bool flipFlop )
		{
            ...
            if (!IncludeUrl) {
				string caption = entity.Market.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Market.ToString();
			writer.Write( "<a href=\"{0}?Market={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Market.Id 
				);
		}
