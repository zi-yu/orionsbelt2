...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// </summary>
	public class ForumReadMarkingThread : BaseFieldControl<ForumReadMarking>, INamingContainer {
		...
		protected override void Render( HtmlTextWriter writer, ForumReadMarking entity, int renderCount, bool flipFlop )
		{
            ...
            if (!IncludeUrl) {
				string caption = entity.Thread.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Thread.ToString();
			writer.Write( "<a href=\"{0}?ForumThread={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Thread.Id 
				);
		}
