...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// <summary>
    /// Renders in XHTML the content of a CampaignStatus's LevelTemplate
    /// </summary>
	public class CampaignStatusLevelTemplate : BaseFieldControl<CampaignStatus>, INamingContainer {
		...
		{
			if ( entity.LevelTemplate == null ) {
                return;
            }
            if (!IncludeUrl) {
				string caption = entity.LevelTemplate.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.LevelTemplate.ToString();
			writer.Write( "<a href=\"{0}?CampaignLevel={2}\">{1}</a>", 
						Url, urlToShow,
						entity.LevelTemplate.Id 
				);
		}
