...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the SubCategory of the Message entity
    /// </summary>
	public class MessageSubCategoryEditor : 
		...
		{
			if( !Secret ) {
				entity.SubCategory = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.SubCategory = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_subCategory", ID); }
		}
