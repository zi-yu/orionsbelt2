...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Extended of the Message entity
    /// </summary>
	public class MessageExtendedEditor : 
		...
		{
			if( !Secret ) {
				entity.Extended = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Extended = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_extended", ID); }
		}
