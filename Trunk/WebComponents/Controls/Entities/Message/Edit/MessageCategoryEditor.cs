...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Category of the Message entity
    /// </summary>
	public class MessageCategoryEditor : 
		...
		{
			if( !Secret ) {
				entity.Category = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Category = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_category", ID); }
		}
