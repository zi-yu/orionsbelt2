...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// </summary>
	public class AssetTask : BaseFieldControl<Asset>, INamingContainer {
		...
		protected override void Render( HtmlTextWriter writer, Asset entity, int renderCount, bool flipFlop )
		{
            ...
            if (!IncludeUrl) {
				string caption = entity.Task.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Task.ToString();
			writer.Write( "<a href=\"{0}?Task={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Task.Id 
				);
		}
