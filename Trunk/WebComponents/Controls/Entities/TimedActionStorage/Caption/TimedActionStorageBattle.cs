...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// <summary>
    /// Renders in XHTML the content of a TimedActionStorage's Battle
    /// </summary>
	public class TimedActionStorageBattle : BaseFieldControl<TimedActionStorage>, INamingContainer {
		...
		{
			if ( entity.Battle == null ) {
                return;
            }
            if (!IncludeUrl) {
				string caption = entity.Battle.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Battle.ToString();
			writer.Write( "<a href=\"{0}?Battle={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Battle.Id 
				);
		}
