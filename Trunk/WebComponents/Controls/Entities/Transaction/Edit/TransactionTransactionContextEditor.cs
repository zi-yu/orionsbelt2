...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the TransactionContext of the Transaction entity
    /// </summary>
	public class TransactionTransactionContextEditor : 
					StringEditor<Transaction>, INamingContainer {
		...
		protected override string GetCaption( Transaction entity )
		{
			return entity.TransactionContext;
		}
		...
		{
			if( !Secret ) {
				entity.TransactionContext = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.TransactionContext = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_transactionContext", ID); }
		}
