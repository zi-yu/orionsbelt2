...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Modifiers of the PlanetStorage entity
    /// </summary>
	public class PlanetStorageModifiersEditor : 
		...
		{
			if( !Secret ) {
				entity.Modifiers = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Modifiers = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_modifiers", ID); }
		}
