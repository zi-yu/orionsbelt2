...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Richness of the PlanetStorage entity
    /// </summary>
	public class PlanetStorageRichnessEditor : 
		...
		{
			if( !Secret ) {
				entity.Richness = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Richness = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_richness", ID); }
		}
