...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the BotName of the CampaignLevel entity
    /// </summary>
	public class CampaignLevelBotNameEditor : 
		...
		{
			if( !Secret ) {
				entity.BotName = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.BotName = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_botName", ID); }
		}
