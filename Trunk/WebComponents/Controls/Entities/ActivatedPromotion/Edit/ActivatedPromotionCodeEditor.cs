...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Code of the ActivatedPromotion entity
    /// </summary>
	public class ActivatedPromotionCodeEditor : 
					StringEditor<ActivatedPromotion>, INamingContainer {
		...
		protected override string GetCaption( ActivatedPromotion entity )
		{
			return entity.Code;
		}
		...
		{
			if( !Secret ) {
				entity.Code = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Code = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_code", ID); }
		}
