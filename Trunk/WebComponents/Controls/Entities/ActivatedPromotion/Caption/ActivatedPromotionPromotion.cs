...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// </summary>
	public class ActivatedPromotionPromotion : BaseFieldControl<ActivatedPromotion>, INamingContainer {
		...
		protected override void Render( HtmlTextWriter writer, ActivatedPromotion entity, int renderCount, bool flipFlop )
		{
            ...
            if (!IncludeUrl) {
				string caption = entity.Promotion.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Promotion.ToString();
			writer.Write( "<a href=\"{0}?Promotion={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Promotion.Id 
				);
		}
