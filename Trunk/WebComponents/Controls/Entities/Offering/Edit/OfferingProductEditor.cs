...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Product of the Offering entity
    /// </summary>
	public class OfferingProductEditor : 
					StringEditor<Offering>, INamingContainer {
		...
		protected override string GetCaption( Offering entity )
		{
		...
		{
			if( !Secret ) {
				entity.Product = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Product = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_product", ID); }
		}
