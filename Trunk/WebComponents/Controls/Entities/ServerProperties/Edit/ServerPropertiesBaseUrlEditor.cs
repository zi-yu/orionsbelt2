...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the BaseUrl of the ServerProperties entity
    /// </summary>
	public class ServerPropertiesBaseUrlEditor : 
					StringEditor<ServerProperties>, INamingContainer {
		...
		protected override string GetCaption( ServerProperties entity )
		{
		...
		{
			if( !Secret ) {
				entity.BaseUrl = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.BaseUrl = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_baseUrl", ID); }
		}
