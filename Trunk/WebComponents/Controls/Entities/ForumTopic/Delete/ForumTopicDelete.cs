...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Deletes an ForumTopic object from the data source
    /// </summary>
	public class ForumTopicDelete : BaseFieldControl<ForumTopic> {
		...
		protected override void OnLoad(EventArgs e)
        {
            base.OnLoad(e);
            if( Page.IsPostBack ) {
				string toRemove = Page.Request.Form["ForumTopicIdToDelete"];
				if( !string.IsNullOrEmpty(toRemove) && HttpContext.Current.Items["ForumTopicIdToDelete"] == null ) {
					HttpContext.Current.Items["ForumTopicIdToDelete"] = string.Empty;
					DeleteForumTopic ( int.Parse(toRemove) );
				}
			}
			RegisterDeleteJS();
        }
		...
		/// <summary>
        /// Deletes a ForumTopic instance from the data source
        /// </summary>
        /// <param name="id">The ForumTopic Id</param>
        private void DeleteForumTopic ( int id )
        {
			using( IForumTopicPersistance persistance = Persistance.Instance.GetForumTopicPersistance() ) {
				ForumTopic entity = persistance.Select(id);
        ...
        /// <summary>
        /// Deletes a ForumTopic's OneToMany relations from the data source
        /// </summary>
        /// <param name="entity">Target entity</param>
        private void DeleteOneToMany( IPersistanceSession session, ForumTopic entity )
        {
		...
		/// <summary>
        /// Renders an ForumTopic delete link
        /// </summary>
		...
		{
            writer.Write("<a href='javascript:DeleteForumTopic({0})'>{1}</a>", entity.Id.ToString(), Text);
		}
    ...
    function DeleteForumTopic ( id )
    {
        if (!theForm.onsubmit || (theForm.onsubmit() != false)) {
		        theForm.ForumTopicIdToDelete.value = id;
		        theForm.submit();
		    }
	}
	</script>";
	
			Page.ClientScript.RegisterClientScriptBlock(typeof(ForumTopic),"DeleteForumTopic", script);
			Page.ClientScript.RegisterHiddenField("ForumTopicIdToDelete", "");
        }
