...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Priority of the Task entity
    /// </summary>
	public class TaskPriorityEditor : 
					StringEditor<Task>, INamingContainer {
		...
		protected override string GetCaption( Task entity )
		{
		...
		{
			if( !Secret ) {
				entity.Priority = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Priority = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_priority", ID); }
		}
