...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// </summary>
	public class TaskNecessity : BaseFieldControl<Task>, INamingContainer {
		...
		{
			if ( entity.Necessity == null ) {
                return;
            }
            if (!IncludeUrl) {
				string caption = entity.Necessity.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Necessity.ToString();
			writer.Write( "<a href=\"{0}?Necessity={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Necessity.Id 
				);
		}
