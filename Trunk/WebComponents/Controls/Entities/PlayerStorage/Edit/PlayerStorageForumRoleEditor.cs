...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the ForumRole of the PlayerStorage entity
    /// </summary>
	public class PlayerStorageForumRoleEditor : 
		...
		{
			if( !Secret ) {
				entity.ForumRole = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.ForumRole = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_forumRole", ID); }
		}
