...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the AllianceRank of the PlayerStorage entity
    /// </summary>
	public class PlayerStorageAllianceRankEditor : 
		...
		{
			if( !Secret ) {
				entity.AllianceRank = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.AllianceRank = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_allianceRank", ID); }
		}
