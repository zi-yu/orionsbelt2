...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the GeneralName of the PlayerStorage entity
    /// </summary>
	public class PlayerStorageGeneralNameEditor : 
		...
		{
			if( !Secret ) {
				entity.GeneralName = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.GeneralName = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_generalName", ID); }
		}
