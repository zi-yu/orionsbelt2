...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Signature of the PlayerStorage entity
    /// </summary>
	public class PlayerStorageSignatureEditor : 
		...
		{
			if( !Secret ) {
				entity.Signature = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Signature = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_signature", ID); }
		}
