...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Race of the PlayerStorage entity
    /// </summary>
	public class PlayerStorageRaceEditor : 
		...
		{
			if( !Secret ) {
				entity.Race = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Race = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_race", ID); }
		}
