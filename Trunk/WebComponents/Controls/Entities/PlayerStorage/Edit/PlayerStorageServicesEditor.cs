...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Services of the PlayerStorage entity
    /// </summary>
	public class PlayerStorageServicesEditor : 
		...
		{
			if( !Secret ) {
				entity.Services = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Services = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_services", ID); }
		}
