...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the TournamentMode of the Battle entity
    /// </summary>
	public class BattleTournamentModeEditor : 
		...
		{
			if( !Secret ) {
				entity.TournamentMode = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.TournamentMode = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_tournamentMode", ID); }
		}
