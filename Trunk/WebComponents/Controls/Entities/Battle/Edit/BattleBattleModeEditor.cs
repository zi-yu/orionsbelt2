...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the BattleMode of the Battle entity
    /// </summary>
	public class BattleBattleModeEditor : 
		...
		{
			if( !Secret ) {
				entity.BattleMode = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.BattleMode = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_battleMode", ID); }
		}
