...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Terrain of the Battle entity
    /// </summary>
	public class BattleTerrainEditor : 
		...
		{
			if( !Secret ) {
				entity.Terrain = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Terrain = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_terrain", ID); }
		}
