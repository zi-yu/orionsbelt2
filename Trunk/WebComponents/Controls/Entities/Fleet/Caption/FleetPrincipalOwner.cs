...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// <summary>
    /// Renders in XHTML the content of a Fleet's PrincipalOwner
    /// </summary>
	public class FleetPrincipalOwner : BaseFieldControl<Fleet>, INamingContainer {
		...
		{
			if ( entity.PrincipalOwner == null ) {
                return;
            }
            if (!IncludeUrl) {
				string caption = entity.PrincipalOwner.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.PrincipalOwner.ToString();
			writer.Write( "<a href=\"{0}?Principal={2}\">{1}</a>", 
						Url, urlToShow,
						entity.PrincipalOwner.Id 
				);
		}
