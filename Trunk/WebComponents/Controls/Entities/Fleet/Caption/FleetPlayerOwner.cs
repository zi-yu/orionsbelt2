...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// </summary>
	public class FleetPlayerOwner : BaseFieldControl<Fleet>, INamingContainer {
		...
		{
			if ( entity.PlayerOwner == null ) {
                return;
            }
            if (!IncludeUrl) {
				string caption = entity.PlayerOwner.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.PlayerOwner.ToString();
			writer.Write( "<a href=\"{0}?PlayerStorage={2}\">{1}</a>", 
						Url, urlToShow,
						entity.PlayerOwner.Id 
				);
		}
