...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Cargo of the Fleet entity
    /// </summary>
	public class FleetCargoEditor : 
					StringEditor<Fleet>, INamingContainer {
		...
		protected override string GetCaption( Fleet entity )
		{
			return entity.Cargo;
		}
		...
		{
			if( !Secret ) {
				entity.Cargo = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Cargo = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_cargo", ID); }
		}
