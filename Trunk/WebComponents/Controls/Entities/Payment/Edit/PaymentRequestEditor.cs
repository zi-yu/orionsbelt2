...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Request of the Payment entity
    /// </summary>
	public class PaymentRequestEditor : 
		...
		{
			if( !Secret ) {
				entity.Request = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Request = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_request", ID); }
		}
