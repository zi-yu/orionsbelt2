...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Ammount of the Payment entity
    /// </summary>
	public class PaymentAmmountEditor : 
		...
		{
			if( !Secret ) {
				entity.Ammount = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Ammount = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_ammount", ID); }
		}
