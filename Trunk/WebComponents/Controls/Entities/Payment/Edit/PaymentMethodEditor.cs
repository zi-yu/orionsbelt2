...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Method of the Payment entity
    /// </summary>
	public class PaymentMethodEditor : 
		...
		{
			if( !Secret ) {
				entity.Method = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Method = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_method", ID); }
		}
