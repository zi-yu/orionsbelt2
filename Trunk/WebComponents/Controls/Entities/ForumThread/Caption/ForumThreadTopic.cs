...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// <summary>
    /// Renders in XHTML the content of a ForumThread's Topic
    /// </summary>
	public class ForumThreadTopic : BaseFieldControl<ForumThread>, INamingContainer {
		...
		protected override void Render( HtmlTextWriter writer, ForumThread entity, int renderCount, bool flipFlop )
		{
            ...
            if (!IncludeUrl) {
				string caption = entity.Topic.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Topic.ToString();
			writer.Write( "<a href=\"{0}?ForumTopic={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Topic.Id 
				);
		}
