...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the TeamName of the PlayerBattleInformation entity
    /// </summary>
	public class PlayerBattleInformationTeamNameEditor : 
		...
		{
			if( !Secret ) {
				entity.TeamName = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.TeamName = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_teamName", ID); }
		}
