...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Details of the AuctionHouse entity
    /// </summary>
	public class AuctionHouseDetailsEditor : 
					StringEditor<AuctionHouse>, INamingContainer {
		...
		protected override string GetCaption( AuctionHouse entity )
		{
		...
		{
			if( !Secret ) {
				entity.Details = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Details = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_details", ID); }
		}
