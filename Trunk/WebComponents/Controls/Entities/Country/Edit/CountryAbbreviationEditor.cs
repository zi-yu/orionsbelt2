...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Abbreviation of the Country entity
    /// </summary>
	public class CountryAbbreviationEditor : 
					StringEditor<Country>, INamingContainer {
		...
		protected override string GetCaption( Country entity )
		{
		...
		{
			if( !Secret ) {
				entity.Abbreviation = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Abbreviation = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_abbreviation", ID); }
		}
