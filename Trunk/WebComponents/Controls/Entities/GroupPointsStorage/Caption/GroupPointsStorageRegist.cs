...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// <summary>
    /// Renders in XHTML the content of a GroupPointsStorage's Regist
    /// </summary>
	public class GroupPointsStorageRegist : BaseFieldControl<GroupPointsStorage>, INamingContainer {
		...
		{
			if ( entity.Regist == null ) {
                return;
            }
            if (!IncludeUrl) {
				string caption = entity.Regist.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Regist.ToString();
			writer.Write( "<a href=\"{0}?PrincipalTournament={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Regist.Id 
				);
		}
