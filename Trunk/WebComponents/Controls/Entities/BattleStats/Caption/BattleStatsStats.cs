...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// </summary>
	public class BattleStatsStats : BaseFieldControl<BattleStats>, INamingContainer {
		...
		protected override void Render( HtmlTextWriter writer, BattleStats entity, int renderCount, bool flipFlop )
		{
            ...
            if (!IncludeUrl) {
				string caption = entity.Stats.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Stats.ToString();
			writer.Write( "<a href=\"{0}?PrincipalStats={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Stats.Id 
				);
		}
