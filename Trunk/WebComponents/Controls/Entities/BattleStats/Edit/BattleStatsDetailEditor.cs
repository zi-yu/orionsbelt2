...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Detail of the BattleStats entity
    /// </summary>
	public class BattleStatsDetailEditor : 
					StringEditor<BattleStats>, INamingContainer {
		...
		protected override string GetCaption( BattleStats entity )
		{
		...
		{
			if( !Secret ) {
				entity.Detail = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Detail = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_detail", ID); }
		}
