...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the ShortName of the VoteReferral entity
    /// </summary>
	public class VoteReferralShortNameEditor : 
					StringEditor<VoteReferral>, INamingContainer {
		...
		protected override string GetCaption( VoteReferral entity )
		{
		...
		{
			if( !Secret ) {
				entity.ShortName = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.ShortName = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_shortName", ID); }
		}
