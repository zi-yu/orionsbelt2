...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Reward of the VoteReferral entity
    /// </summary>
	public class VoteReferralRewardEditor : 
		...
		{
			if( !Secret ) {
				entity.Reward = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Reward = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_reward", ID); }
		}
