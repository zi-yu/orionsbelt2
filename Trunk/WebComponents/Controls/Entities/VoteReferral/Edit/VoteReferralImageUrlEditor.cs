...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the ImageUrl of the VoteReferral entity
    /// </summary>
	public class VoteReferralImageUrlEditor : 
		...
		{
			if( !Secret ) {
				entity.ImageUrl = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.ImageUrl = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_imageUrl", ID); }
		}
