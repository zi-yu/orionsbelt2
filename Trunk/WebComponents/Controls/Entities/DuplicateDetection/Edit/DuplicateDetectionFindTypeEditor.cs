...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the FindType of the DuplicateDetection entity
    /// </summary>
	public class DuplicateDetectionFindTypeEditor : 
		...
		{
			if( !Secret ) {
				entity.FindType = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.FindType = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_findType", ID); }
		}
