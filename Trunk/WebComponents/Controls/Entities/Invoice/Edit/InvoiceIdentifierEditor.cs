...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Identifier of the Invoice entity
    /// </summary>
	public class InvoiceIdentifierEditor : 
					StringEditor<Invoice>, INamingContainer {
		...
		protected override string GetCaption( Invoice entity )
		{
		...
		{
			if( !Secret ) {
				entity.Identifier = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Identifier = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_identifier", ID); }
		}
