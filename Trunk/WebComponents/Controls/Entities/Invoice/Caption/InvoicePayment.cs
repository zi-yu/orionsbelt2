...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// </summary>
	public class InvoicePayment : BaseFieldControl<Invoice>, INamingContainer {
		...
		protected override void Render( HtmlTextWriter writer, Invoice entity, int renderCount, bool flipFlop )
		{
            ...
            if (!IncludeUrl) {
				string caption = entity.Payment.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Payment.ToString();
			writer.Write( "<a href=\"{0}?Payment={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Payment.Id 
				);
		}
