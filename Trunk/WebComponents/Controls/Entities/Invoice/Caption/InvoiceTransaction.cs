...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// </summary>
	public class InvoiceTransaction : BaseFieldControl<Invoice>, INamingContainer {
		...
		protected override void Render( HtmlTextWriter writer, Invoice entity, int renderCount, bool flipFlop )
		{
            ...
            if (!IncludeUrl) {
				string caption = entity.Transaction.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Transaction.ToString();
			writer.Write( "<a href=\"{0}?Transaction={2}\">{1}</a>", 
						Url, urlToShow,
