...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Coordinates of the Market entity
    /// </summary>
	public class MarketCoordinatesEditor : 
					StringEditor<Market>, INamingContainer {
		...
		protected override string GetCaption( Market entity )
		{
		...
		{
			if( !Secret ) {
				entity.Coordinates = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Coordinates = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_coordinates", ID); }
		}
