...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Server of the BotCredential entity
    /// </summary>
	public class BotCredentialServerEditor : 
		...
		{
			if( !Secret ) {
				entity.Server = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Server = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_server", ID); }
		}
