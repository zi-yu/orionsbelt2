...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Currency of the PaymentDescription entity
    /// </summary>
	public class PaymentDescriptionCurrencyEditor : 
					StringEditor<PaymentDescription>, INamingContainer {
		...
		protected override string GetCaption( PaymentDescription entity )
		{
		...
		{
			if( !Secret ) {
				entity.Currency = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Currency = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_currency", ID); }
		}
