...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Language of the AllianceStorage entity
    /// </summary>
	public class AllianceStorageLanguageEditor : 
		...
		{
			if( !Secret ) {
				entity.Language = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Language = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_language", ID); }
		}
