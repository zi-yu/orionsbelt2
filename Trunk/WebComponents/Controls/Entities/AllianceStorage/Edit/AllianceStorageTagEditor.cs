...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Tag of the AllianceStorage entity
    /// </summary>
	public class AllianceStorageTagEditor : 
		...
		{
			if( !Secret ) {
				entity.Tag = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Tag = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_tag", ID); }
		}
