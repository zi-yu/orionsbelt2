...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Slot of the PlanetResourceStorage entity
    /// </summary>
	public class PlanetResourceStorageSlotEditor : 
		...
		{
			if( !Secret ) {
				entity.Slot = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Slot = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_slot", ID); }
		}
