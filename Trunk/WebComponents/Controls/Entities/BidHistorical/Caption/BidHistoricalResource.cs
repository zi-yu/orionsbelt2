...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// <summary>
    /// Renders in XHTML the content of a BidHistorical's Resource
    /// </summary>
	public class BidHistoricalResource : BaseFieldControl<BidHistorical>, INamingContainer {
		...
		protected override void Render( HtmlTextWriter writer, BidHistorical entity, int renderCount, bool flipFlop )
		{
            ...
            if (!IncludeUrl) {
				string caption = entity.Resource.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Resource.ToString();
			writer.Write( "<a href=\"{0}?AuctionHouse={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Resource.Id 
				);
		}
