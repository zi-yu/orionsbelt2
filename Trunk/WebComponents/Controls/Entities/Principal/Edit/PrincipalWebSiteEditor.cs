...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the WebSite of the Principal entity
    /// </summary>
	public class PrincipalWebSiteEditor : 
		...
		{
			if( !Secret ) {
				entity.WebSite = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.WebSite = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_webSite", ID); }
		}
