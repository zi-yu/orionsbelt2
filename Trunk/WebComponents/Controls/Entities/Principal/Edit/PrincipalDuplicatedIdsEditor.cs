...
namespace OrionsBelt.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the DuplicatedIds of the Principal entity
    /// </summary>
	public class PrincipalDuplicatedIdsEditor : 
		...
		{
			if( !Secret ) {
				entity.DuplicatedIds = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.DuplicatedIds = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_duplicatedIds", ID); }
		}
