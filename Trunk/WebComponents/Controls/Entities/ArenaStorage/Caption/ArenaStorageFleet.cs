...
namespace OrionsBelt.WebComponents.Controls {
    ...
    /// <summary>
    /// Renders in XHTML the content of a ArenaStorage's Fleet
    /// </summary>
	public class ArenaStorageFleet : BaseFieldControl<ArenaStorage>, INamingContainer {
		...
		protected override void Render( HtmlTextWriter writer, ArenaStorage entity, int renderCount, bool flipFlop )
		{
            ...
            if (!IncludeUrl) {
				string caption = entity.Fleet.ToString();
                writer.Write( caption );
                return;
            }
			string urlToShow = urlText != string.Empty ? urlText : entity.Fleet.ToString();
			writer.Write( "<a href=\"{0}?Fleet={2}\">{1}</a>", 
						Url, urlToShow,
						entity.Fleet.Id 
				);
		}
