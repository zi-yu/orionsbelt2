...
namespace OrionsBelt.DataAccessLayer {
	...
	public class NHibernatePersistanceStateless<T> :
			IPersistance<T>, 
			ILifecycle<T>,
			IDisposable 
			where T : IEntity {
		...
		private IStatelessSession session = null;
		...
		protected NHibernatePersistanceStateless ( IStatelessSession currSession, Type type ) 
		{
        ...
        /// </summary>
		internal IStatelessSession NHibernateSession {
			get { 
				if (session == null || !NHibernateUtilities.IsSessionOpen(session)) {
                    session = NHibernateUtilities.OpenStatelessSession;
                }
				return session; 
			}
			set { session = value; }
        ...
        public void Reset()
        {
			...
			NHibernateSession = NHibernateUtilities.OpenStatelessSession;
        }
		...
		public IList<T> Select( int start, int count, string[] orderByProperties, bool[] ascending )
		{
			...
			ICriteria criteria = NHibernateSession.CreateCriteria(PersistanceType.Name);
        ...
        public IList MultiQuery(IEnumerable<string> hqls, Dictionary<string, object> args)
        {
            ...
            foreach (string hql in hqls) {
                IQuery query = NHibernateSession.CreateQuery(hql);
                if (args != null) {
