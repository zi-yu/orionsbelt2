...
namespace OrionsBelt.DataAccessLayer {
	...
	/// <summary>
    /// Specialized NHibernate class for Payment
    /// </summary>
	public class SpecializedPayment : Payment {
				...
				if( this.invoice == null ) {
					if (Transient) {
						...
						using( IInvoicePersistance persistance = Persistance.Instance.GetInvoicePersistance ()) {
							this.invoice = persistance.SelectByPayment (this);
						}
						if( this.invoice == null ) {
							this.invoice = new List<Invoice>();
						}
		...
		/// <summary>
        /// Gets the Payment's Invoice NHibernate container
        /// </summary>
		[XmlIgnore()]
