...
namespace Institutional.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Password of the Principal entity
    /// </summary>
	public class PrincipalPasswordEditor : 
		...
		{
			if( !Secret ) {
				entity.Password = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Password = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_password", ID); }
		}
		...
		protected virtual void AddValidators()
		{
            ...
            maxSize.ValidationExpression = "(.|\\s){1,50}";
            //maxSize.ErrorMessage = string.Format(Resources.LengthValidatorToken, 50);
			...
			if (GetParent().Source == SourceType.New) {
				...
				//secretreq.ErrorMessage = string.Format(Resources.RequiredFieldValidatorToken, Resources.PrincipalPasswordToken);
			}
		}
