...
namespace Institutional.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Content of the Testimonial entity
    /// </summary>
	public class TestimonialContentEditor : 
					StringEditor<Testimonial>, INamingContainer {
		...
		protected override string GetCaption( Testimonial entity )
		{
		...
		{
			if( !Secret ) {
				entity.Content = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Content = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_content", ID); }
		}
