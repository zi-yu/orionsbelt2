...
namespace Institutional.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Author of the Testimonial entity
    /// </summary>
	public class TestimonialAuthorEditor : 
		...
		{
			if( !Secret ) {
				entity.Author = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Author = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_author", ID); }
		}
