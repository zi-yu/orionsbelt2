...
namespace Institutional.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the Exceprt of the MediaArticle entity
    /// </summary>
	public class MediaArticleExceprtEditor : 
					StringEditor<MediaArticle>, INamingContainer {
		...
		protected override string GetCaption( MediaArticle entity )
		{
			return entity.Exceprt;
		}
		...
		{
			if( !Secret ) {
				entity.Exceprt = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.Exceprt = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_exceprt", ID); }
		}
