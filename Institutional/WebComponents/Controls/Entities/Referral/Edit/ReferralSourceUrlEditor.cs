...
namespace Institutional.WebComponents.Controls {
	...
	/// <summary>
    /// Edits the SourceUrl of the Referral entity
    /// </summary>
	public class ReferralSourceUrlEditor : 
					StringEditor<Referral>, INamingContainer {
		...
		protected override string GetCaption( Referral entity )
		{
		...
		{
			if( !Secret ) {
				entity.SourceUrl = text.Text;
			}else{
				if( !string.IsNullOrEmpty(text.Text) ) {
					entity.SourceUrl = FormsAuthentication.HashPasswordForStoringInConfigFile( text.Text, "sha1" );
				}
			}
		}
		...
		protected override string TargetName { 
			get { return string.Format("Mid_StringEditor_{0}_sourceUrl", ID); }
		}
